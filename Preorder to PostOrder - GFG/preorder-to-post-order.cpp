...
public:
    Node* post_order(int pre[], int size) {
        ...
        // Call the preToBST function to convert the given preorder array to a binary search tree
        // Start with initial minimum and maximum values of INT_MIN and INT_MAX respectively
    ...
    Node* preToBST(int pre[], int& i, int size, int mini, int maxi) {
        // Base case: If all elements of the preorder array have been processed, return NULL
        if (i >= size) {
            return NULL;
        }
        
        // Get the current element from the preorder array
        int x = pre[i];
        
        // If the current element is less than the minimum value or greater than the maximum value,
        ...
        Node* root = newNode(x);
        i++; // Move to the next element in the preorder array
        
        // Recursively construct the left subtree by passing the updated minimum and current value as the maximum
        root->left = preToBST(pre, i, size, mini, x);
        
        // Recursively construct the right subtree by passing the current value as the minimum and the updated maximum
